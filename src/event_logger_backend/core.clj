...
(comment
  ...
  (start-server! 8000 "localhost")

  (stop-server!)

  (app {:scheme :http :request-method :get :uri "/storage/api/logger/x"})

  (app {:scheme :http :request-method :get :uri "/storage/api/logger/events"})

  (app {:scheme :http :request-method :get :uri "/storage/api/ping"})
